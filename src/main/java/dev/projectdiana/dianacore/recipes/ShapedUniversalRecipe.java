...
import static dev.projectdiana.dianacore.scripts.IScriptLoader.missing;
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                ...
                if (r instanceof ItemStack) {
                    if (!Utility.areStacksEqual((ItemStack) r, stack, true)) return false;
                    ...
                    if (!((HashSet<Utility.ItemId>) r).contains(Utility.ItemId.createNoCopy(copy))) {
                        ...
                        // noinspection unchecked
                        if (!((HashSet<Utility.ItemId>) r).contains(Utility.ItemId.createNoCopy(copy))) return false;
                    }
